...
const Signup = () => {
  ...
  const [requestOtp, { isLoading: isOtpLoading }] =
    useRequestRegistrationOtpMutation();
  ...
  const handleSendOtp = async () => {
    ...
    if (!email) {
      setOtpFeedback("Enter your email first to receive an OTP.");
      return;
    }

    try {
      const response = await requestOtp({ email }).unwrap();
      setOtpFeedback(response?.message || "OTP sent to your email.");
    } catch {
      setOtpFeedback("Failed to send OTP. Please try again.");
    }
  };
        ...
        <main className="w-full max-w-md bg-white rounded-lg shadow-lg p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              ...
              className={`w-full py-2.5 border border-indigo-600 text-indigo-600 rounded-md font-medium hover:bg-indigo-50 transition-colors ${
                isOtpLoading ? "cursor-not-allowed opacity-70" : ""
              }`}
            >
            ...
            {otpFeedback && (
              <p className="text-xs text-center text-gray-600">{otpFeedback}</p>
            )}
