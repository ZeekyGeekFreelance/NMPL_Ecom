...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  const { data: searchProductsData, loading: isSearching } = useQuery(
    GET_PRODUCTS,
    {
      variables: {
        first: 48,
  ...
  const rankedProducts = useMemo(() => {
    ...
    const products = (searchProductsData?.products?.products || []) as Product[];
    const rows = products
      .map((product) => ({
        product,
  ...
  }, [debouncedQuery, searchProductsData?.products?.products, shouldSearch]);
