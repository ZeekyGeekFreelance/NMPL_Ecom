...
const runLocalPackageSanity = () => {
  ...
  if (presentLockfiles.length !== 1 || presentLockfiles[0] !== "package-lock.json") {
    throw new Error(
      `[repo-sanity] client package must use only package-lock.json (found: ${presentLockfiles.join(", ") || "none"})`
    );
  }

  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"));
  if (!packageJson.engines || typeof packageJson.engines.node !== "string") {
    throw new Error("[repo-sanity] client package.json must define engines.node");
  }
