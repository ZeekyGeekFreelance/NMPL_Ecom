...
const withPlatformSubject = (subject) => `${platformName} | ${subject}`;
const baseEmailLayout = ({ preview, title, bodyHtml, }) => `
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
const buildDealerApplicationSubmittedEmail = ({ recipientName, businessName, accountReference, portalUrl, supportEmail, wasResubmission = false, }) => {
    const salutation = (recipientName === null || recipientName === void 0 ? void 0 : recipientName.trim()) || "Dealer";
...
exports.buildDealerApplicationSubmittedEmail = buildDealerApplicationSubmittedEmail;
const buildDealerStatusUpdatedEmail = ({ recipientName, businessName, accountReference, status, reviewedBy, portalUrl, supportEmail, }) => {
    const salutation = (recipientName === null || recipientName === void 0 ? void 0 : recipientName.trim()) || "Dealer";
...
exports.buildDealerStatusUpdatedEmail = buildDealerStatusUpdatedEmail;
const buildDealerPricingUpdatedEmail = ({ recipientName, businessName, accountReference, updatedBy, changeCount, totalMappedVariants, changes, portalUrl, supportEmail, }) => {
    const salutation = (recipientName === null || recipientName === void 0 ? void 0 : recipientName.trim()) || "Dealer";
    ...
    const tableRowsHtml = visibleRows
        ...
        <tr>
          ...
          <td style="padding:8px;border:1px solid #e5e7eb;">${formatOptionalCurrency(row.previousPrice)}</td>
          <td style="padding:8px;border:1px solid #e5e7eb;">${formatOptionalCurrency(row.nextPrice)}</td>
        </tr>
      `)
        .join("");
    return {
        ...
        text: [
            ...
            "",
            "Recent changes:",
            ...visibleRows.map((row) => `- ${row.productName} (${row.sku}): ${formatOptionalCurrency(row.previousPrice)} -> ${formatOptionalCurrency(row.nextPrice)}`),
        ...
        ${visibleRows.length
            ...
            `
                : ""}
        ${hasMore
                ? `<p style="margin:0 0 12px;color:#6b7280;">...and ${changes.length - visibleRows.length} more changes.</p>`
                : ""}
        <p style="margin:0;">
          Portal: <a href="${portalUrl}" style="color:#2563eb;">${portalUrl}</a><br />
          Support: <a href="mailto:${supportEmail}" style="color:#2563eb;">${supportEmail}</a>
...
exports.buildDealerPricingUpdatedEmail = buildDealerPricingUpdatedEmail;
const buildDealerAccountCreatedEmail = ({ recipientName, businessName, accountReference, email, temporaryPassword, portalUrl, supportEmail, }) => {
    const salutation = (recipientName === null || recipientName === void 0 ? void 0 : recipientName.trim()) || "Dealer";
    return {
...
exports.buildDealerAccountCreatedEmail = buildDealerAccountCreatedEmail;
const buildDealerRemovedEmail = ({ recipientName, businessName, accountReference, removedBy, supportEmail, }) => {
    const salutation = (recipientName === null || recipientName === void 0 ? void 0 : recipientName.trim()) || "Dealer";
    return {
...
exports.buildDealerRemovedEmail = buildDealerRemovedEmail;
